...
Fixpoint split_helper1 {X Y : Type} (l : list (X*Y))
               : (list X) :=
  match l with
  | nil => nil
  | (x,y) :: t => x :: (split_helper1 t)
  end.

Fixpoint split_helper2 {X Y : Type} (l : list (X*Y))
               : (list Y) :=
  match l with
  | nil => nil
  | (x,y) :: t => y :: (split_helper2 t)
  end.
...
Lemma map_rev_lemma : forall (X Y : Type) (f : X -> Y) (l : list X) (n : X),
